...
class Botao:
    ...
    def exec(self, window):
        ...
        pygame.draw.rect(window, (0, 0, 0), [self.pos[0], self.pos[1], self.size[0], self.size[1]])
        pygame.draw.rect(window, self.currentColor, [self.pos[0] + 5, self.pos[1] + 5, self.size[0] - 10, self.size[1] - 10])
...
class BotaoDeslizante:
    def __init__(self, pos, text, unselectedColor, selectedColor, min, max):
        ...
        self.currentValue = min#self.map_range(self.buttonPos[0], self.pos[0], self.pos[0] + self.size[0], self.min, self.max)#min
    ...
    def exec(self, window):
        ...
        pygame.draw.rect(window, (115, 115, 115), [self.pos[0], self.pos[1], self.size[0], self.size[1]])
        pygame.draw.circle(window, (0, 0, 0), (self.buttonPos[0], self.buttonPos[1]), self.buttonRadius)
        pygame.draw.circle(window, self.currentColor, (self.buttonPos[0], self.buttonPos[1]), self.buttonRadius - 4)
        self.currentValue = self.map_range(self.buttonPos[0], self.pos[0], self.pos[0] + self.size[0], self.min, self.max)
...
class GUI:#controle de parametros do pendulo
    def __init__(self):
        self.enterButton = Botao((WINDOW_WIDTH - 400 + 50, 650), (300, 100), 'Enter', (112, 250, 37), (76, 168, 25))
        self.comprimentoPendulo = BotaoDeslizante((WINDOW_WIDTH - 400 + 50, 300), 'Comprimento (m)', (7, 242, 70), (3, 163, 46), 0.01, 0.7)
        self.amplitudePendulo = BotaoDeslizante((WINDOW_WIDTH - 400 + 50, 500), 'Amplitude (rad)', (7, 242, 70), (3, 163, 46), 0.01, math.pi / 2)
    def exec(self, window):
        pygame.draw.rect(window, (0, 0, 0), [WINDOW_WIDTH - 400, 5, 395, 790])
        pygame.draw.rect(window, (146, 232, 183), [WINDOW_WIDTH - 395, 10, 385, 780])
...
class Pendulo:#x(t) = A cos(sqrt(g/L)t); x(0) = A
    ...
    def draw(self, window):  # desenha pendulo
        ...
        pygame.draw.circle(window, (3, 252, 165), (self.x, self.y), self.raio_bola - 2)
    ...
    def calcula_posicao(self): 
        ...
        self.x = POS_INICIAL[0] + self.comprimento * UM_METRO * math.sin(angulo)
        self.y = POS_INICIAL[1] + self.comprimento * UM_METRO * math.cos(angulo)
...
class Main:
    def __init__(self):
        ...
        self.pendulo = Pendulo(0.248, math.pi / 6, pygame.time.get_ticks() / MILISEGUNDO_PARA_SEGUNDO)#0.248 metros para ter periodo de 1 segundo, pi/6. max:0.7 para comprimento
        self.gui = GUI()
